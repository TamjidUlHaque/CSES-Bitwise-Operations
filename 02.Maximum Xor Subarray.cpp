...
____________________Bitwise_________operations____________________________
...
long long range_or(long long L, long long R) {
    ...
    for (int pos = 62; pos >= 0; pos--) {
        ...
        if (L & mask) ans |= mask;
        else if (R & mask) ans |= mask;
        else {
            if ((L >> (pos+1)) != (R >> (pos+1))) ans |= mask;
        }
    }
    return ans;
}
...
__________________________Combinatorics_________________________________
...
public:
    ...
    int get_max_xor(int number){
        ...
        Node* temp = root ; int answer = 0 ;

        for(int i = 31 ; i >= 0 ; i--){
            int bit = ((number >> i) & 1) ;

            if(!temp->present(bit^1)){
                temp = temp->move(bit) ;
            }else{
                answer += (1 << i) ;
                temp = temp->move(bit^1) ;
            }
        }
...
void IcpcDhakaRegionalist2025(){
    ...
    int n ; cin >> n ; vector<int> arr(n+1,0) ;
    Trie memory ;
    ...
    int ans = *max_element(arr.begin()+1,arr.end()) ;

    for(int i = 0 ; i < n ; i++){
        int cur = memory.get_max_xor(arr[i]) ;
        ans = max(ans,cur) ;
    }
